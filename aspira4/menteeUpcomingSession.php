...
echo "<button class='join-btn' type='button' onclick='showJoinModal(\"The session hasn’t started yet. Please wait!\", \"wait\")'>Join</button>";
        ...
        onclick="openUpdateModal(<?= $session_id ?>)" 
...
<div id="updateModal" style="display:none; position:fixed; top:0; left:0; width:100%; height:100%; background:rgba(0,0,0,0.5); justify-content:center; align-items:center; z-index:2000;">
  <div class="add-session-form">
    ...
    <div id="slotsContainer" class="time-slots-container"></div>
    <button id="updateSaveBtn" class="add-btn" style="margin-top:10px;" onclick="saveUpdate()" disabled>Save</button>
    <button class="add-btn" style="background:#ccc; color:#333; margin-top:10px;" onclick="closeUpdateModal()">Cancel</button>
  </div>
</div>
...
function showJoinModal(message, type = "wait") {
    ...
    if (type === "wait") {
        icon.src = "https://cdn-icons-png.flaticon.com/512/9625/9625452.png"; // أيقونة الانتظار
...
/* ==================  تعديل الجلسة باختيار Slot ================== */

let __editSessionId = null;
let __selectedDate  = null;
let __selectedTime  = null;

// فتح مودال السلوطات وجلب الأوقات المتاحة لنفس المنتور
function openUpdateModal(sessionId) {
    __editSessionId = sessionId;
    __selectedDate  = null;
    __selectedTime  = null;
    ...
    container.innerHTML = "<p style='text-align:center; width:100%;'>Loading available slots...</p>";
    saveBtn.disabled = true;

    modal.style.display = "flex";

    fetch("mentee_load_slots.php?session_id=" + encodeURIComponent(sessionId))

        .then(r => r.json())
        .then(data => {
            if (!data || !data.length) {
                container.innerHTML = "<p style='text-align:center; width:100%; color:#4b398e;'>No available slots for this mentor.</p>";
                return;
            }
            ...
            data.forEach(slot => {
                ...
                div.innerHTML = `
                    <div>${slot.date}</div>
                    <div>${slot.time}</div>
                    <div style="font-size:12px; opacity:0.8;">45 min</div>
                `;

                div.addEventListener("click", function () {
                    document
                        .querySelectorAll("#slotsContainer .time-slot")
                        .forEach(s => s.classList.remove("selected"));

                    this.classList.add("selected");
                    __selectedDate = this.dataset.date;
                    __selectedTime = this.dataset.time;
        ...
        .catch(err => {
            console.error(err);
            container.innerHTML = "<p style='text-align:center; width:100%; color:red;'>Error loading slots.</p>";
        });
}
...
// حفظ التعديل باستخدام mentee_update_session.php
function saveUpdate() {
    if (!__editSessionId || !__selectedDate || !__selectedTime) {
        alert("Please select a slot first");
        return;
    }

    const fd = new FormData();
    fd.append("id",   __editSessionId);
    fd.append("date", __selectedDate);
    fd.append("time", __selectedTime);

    fetch("mentee_reschedule.php", { method: "POST", body: fd })
        .then(r => r.text())
        .then(d => {
            const msg = d.trim();
            if (msg === "success") {
                // تحديث الصف في الجدول بدون ريفريش كامل
                document.querySelector(`#row-${__editSessionId} .date-cell`).textContent = __selectedDate;
                document.querySelector(`#row-${__editSessionId} .time-cell`).textContent = __selectedTime;
                closeUpdateModal();
