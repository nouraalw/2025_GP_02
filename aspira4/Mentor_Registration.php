...
if ($_SERVER["REQUEST_METHOD"] == "POST" && !isset($_POST["check_availability"])) {
    ...
    $check = $conn->prepare("SELECT user_id FROM mentors WHERE user_id = ?");
    $check->bind_param("i", $existing_user_id);
    $check->execute();
    $mentee_res = $check->get_result();

    if ($mentee_res->num_rows == 0) {
        // User exists in USERS but not yet registered as mentor → Insert into mentors table only
        ...
        $stmt2->bind_param("isssssss", $existing_user_id, $email, $cv_path, $field_id, $experience_id, $profile_pic_path, $brief_description, $certificate_path);
        $stmt2->execute();
        $stmt2->close();

        echo "<script>alert('Account completed successfully! Wait for admin approval.');window.location='login.php';</script>";
        exit();
    } else {
...
//  File Upload Handling (Certificates) 
$certificate_paths = [];
