...
$mentee_id = (int)$_SESSION['user_id'];
    ...
    // ===== Public (Group) sessions query =====
$query = "SELECT g.group_session_id, g.session_date, g.session_time, g.room_id,
                 g.title, g.duration,
                 u2.first_name, u2.last_name
          FROM grp_sessions g
          INNER JOIN grp_session_participants p ON g.group_session_id = p.group_session_id
          LEFT JOIN mentors m ON g.mentor_id = m.user_id
          LEFT JOIN users u2 ON m.user_id = u2.user_id
          WHERE p.mentee_id = ?
            AND g.status IN ('booked', 'upcoming')
            AND TIMESTAMP(g.session_date, g.session_time) + INTERVAL COALESCE(g.duration, 45) MINUTE > NOW()
          ORDER BY g.session_date ASC, g.session_time ASC";
    ...
    if (count($rows) > 0) {
        foreach ($rows as $row) {
            ...
            <tr id="row-<?= $session_id ?>" class="<?= $isEnded ? 'session-ended' : '' ?>">
                ...
                <td><?= htmlspecialchars($session_time) ?></td>
                <td><?= htmlspecialchars($duration) ?> min</td>
                <td>
                    <?php
                    $safe_room_id = htmlspecialchars($room_id ?? '', ENT_QUOTES, 'UTF-8');
                    $now = time();
               if ($now >= $start_ts && $now <= $end_ts) {
                echo "<a href='WEB_UIKITS.php?roomID=group_{$session_id}' class='join-btn' target='_blank'>Join</a>";
                  }
                 elseif ($now < $start_ts) {
echo "<button class='join-btn' type='button' onclick='showJoinModal(\"The session hasn’t started yet. Please wait!\", \"wait\")'>Join</button>";
                ...
                <td>
                    <a href="mentee_delete_grp_session.php?id=<?= $session_id ?>&mentee=<?= $mentee_id ?>"
        ...
        echo "<tr><td colspan='7'>There are no upcoming Sessions.</td></tr>";
    }
    ?>
</table>
...
<!-- ✅ Modal Styled like Mentor Version -->
