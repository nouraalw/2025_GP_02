...
if (isset($_GET['delete_id'])) {
    ...
    $del = $conn->prepare("DELETE FROM sessions WHERE id = ? AND mentor_id = ?");
    $del->bind_param("ii", $deleteId, $mentor_id);

    if ($del->execute()) {
        header("Location: addPrivateSession.php?msg=deleted");
        exit();
    } else {
...
.session-table td {
    vertical-align: middle; /* يجعل الزر في النص عمودياً */
    text-align: center;     /* يجعل الزر في النص أفقياً */
}
...
<td style="text-align: center;">
    <a href="addPrivateSession.php?delete_id=<?= (int)$session['id']; ?>"
