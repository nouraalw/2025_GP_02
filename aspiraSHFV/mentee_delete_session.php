...
// ارجاع الجلسة (سواء كانت Group أو Private) إلى متاحة
$restore = $conn->prepare("UPDATE sessions
                           SET mentee_id = NULL, status = 'available', room_id = NULL
                           WHERE id = ? AND mentee_id = ? AND status = 'booked'");
$restore->bind_param("ii", $session_id, $mentee_id);
$restore->execute();
...
try {
    ...
    $mail->isHTML(true);
    $mail->Subject = 'A Session Has Been Canceled by a Mentee';
    $mail->Body    = "
        <h2>Session Canceled</h2>
        <p>Dear Mentor,</p>
        <p>The mentee canceled the session scheduled on <strong>{$session['date']}</strong> at <strong>{$session['time']}</strong>.</p>
...
header("Location: menteeUpcomingSession.php?msg=deleted");
exit;
