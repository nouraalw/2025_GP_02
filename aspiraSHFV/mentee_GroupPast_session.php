...
if (!isset($_SESSION['user_id'])) { header("Location: login.php"); exit(); }
 ...
 $query = "
      SELECT
        ...
        g.session_time AS time,
        g.room_id,
        g.title,
        g.status,
        COALESCE(g.duration,45) AS duration,
        m.mentor_id,
        ...
        f.field_name
      FROM grp_sessions g
      INNER JOIN grp_session_participants p
              ON p.group_session_id = g.group_session_id
      LEFT  JOIN mentors m
              ON m.user_id = g.mentor_id          -- mapping by user_id
      LEFT  JOIN users   u
              ON u.user_id = m.user_id
      LEFT  JOIN field   f
              ON f.id = m.field_id
      WHERE p.mentee_id = ?
        AND g.status IN ('completed','expired')
      ORDER BY g.session_date DESC, g.session_time DESC
    ";
...
.star-rating {
  display: inline-flex;
  flex-direction: row-reverse;   /* نفس طريقتك */
  justify-content: center;
}
.star-rating input {
  display: none;                 /* نخفي الراديو */
}
.star-rating label {
  font-size: 25px;
  color: #ccc;                   /* الرمادي الافتراضي */
  cursor: pointer;
}

/* مهم: اللون الذهبي عند الاختيار */
.star-rating input:checked ~ label {
  color: #f5b301 !important;
}

/* اللون الذهبي عند التحويم */
.star-rating label:hover,
.star-rating label:hover ~ label {
  color: #f5b301;
}

/* لو عطّلتها بعد الإرسال، يظل المظهر واضح */
...
<?php else: ?>
    <?php 
        $colspan = 6; // fixed to match table headers
        echo "<tr><td colspan='{$colspan}'>No past sessions found.</td></tr>";
    ?>
<?php endif; ?>
