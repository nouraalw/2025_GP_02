...
$sql = "
SELECT 
  u.first_name, u.last_name, u.email, u.phone_number, u.role,
...
$stmt->store_result();
$stmt->bind_result(
  $cur_fname, $cur_lname, $cur_email, $cur_phone, $cur_role,
...
// ================= AJAX: check_availability (email/phone) =================
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['check_availability'])) {
  ...
  if (($_POST['check_availability'] ?? '') === 'email') {
    ...
    $stmt->close();
  } elseif (($_POST['check_availability'] ?? '') === 'phone') {
    $phone = trim($_POST['phone'] ?? '');
    $sql = "SELECT 1 FROM users WHERE phone_number=? AND user_id<>? LIMIT 1";
    $stmt = $conn->prepare($sql);
...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['save_mentor'])) {
  ...
  if ($fname==='' || $lname==='' || $email==='' || $phone==='') { die("Please fill all required fields."); }
  ...
  $stmt->bind_param("si", $phone, $user_id);
  $stmt->execute(); $stmt->store_result();
  if ($stmt->num_rows>0){ $stmt->close(); die("Phone already in use."); }
  $stmt->close();
...
$stmt->execute(); $stmt->close();
    ...
    header("Location: edit_profile.php?saved=1"); exit;
  } catch (Throwable $e) {
...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['save_mentee'])) {
  ...
  if ($fname==='' || $lname==='' || $email==='' || $phone==='' || $status==='' || $interests==='') {
...
<div class="content">
  <div class="card">
    ...
    <h2 style="text-align:center;">Edit profile (mentor)</h2>
      ...
      <form id="mentor-form" method="POST" action="edit_profile.php" enctype="multipart/form-data">
           <div class="pfp-wrap">
  ...
  <input type="file" id="pfpInput" name="profile_picture" accept="image/*" hidden>
        ...
        <input type="tel" id="phone" name="phone" value="<?php echo htmlspecialchars($cur_phone); ?>" required>
    ...
    <h2 style="text-align:center;">Edit profile (mentee)</h2>
      ...
      <form id="mentee-form" method="POST" action="edit_profile.php">
        <div class="grid-2">
          <div>
            <label>First Name</label>
            <input type="text" name="first_name" value="<?php echo htmlspecialchars($cur_fname); ?>" required>
